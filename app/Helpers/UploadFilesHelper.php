...
class UploadFilesHelper
{
    ...
    public static function url_to_uploaded_file($file, $filename = null)
    {
        ...
        if (in_array($uploaded_file->getMimeType(), $image_mime_types)) {
            self::appendWaterMark($uploaded_file->getPathname());
        }
    ...
    public static function base64_to_file($file)
    {
        ...
        // Check if the uploaded file is an image
        $image_mime_types = ['image/jpeg', 'image/png', 'image/gif'];
        if (in_array($file->getMimeType(), $image_mime_types)) {
            self::appendWaterMark($file->getPathname());
        }
    ...
    public static function store_file($options)
    {
        ...
        if (null != $options["resize"] && $options['validation'] == "image" && $options['optimize'] == true) {
            ...
            if (isset($options["resize"][0])) {
                $image_sm = \Image::make($image_sm)->resize($options["resize"][0], null, function ($constraint) {
                    ...
                    $constraint->upsize();
                })->insert(Setting::first()->website_logo())->encode($extension);
            }
            $image_sm->stream();
            ...
            if (isset($options['optimize']) && $options['optimize'] == true) {
                ...
                if ($image_lg->width() >= 3000) {
                    $quality = 30;
                } elseif ($image_lg->width() < 3000 && $image_lg->width() >= 2000) {
                    $quality = 40;
                } elseif ($image_lg->width() < 2000 && $image_lg->width() >= 1000) {
                    $quality = 50;
                } else {
                    $quality = 60;
                }
            ...
            try {
                \Storage::disk($options["file_system_type"])->put(strtolower($options['visibility']) . $path_small . $filename, $image_sm, $filename);
            } catch (\Exception $e) {
            }
            try {
                \Storage::disk($options["file_system_type"])->put(strtolower($options['visibility']) . $path . $filename, $image_lg);
            } catch (\Exception $e) {
            }
        } else {
            try {
                \Storage::disk($options["file_system_type"])->putFileAs(strtolower($options['visibility']) . $path, $file, $filename);
            } catch (\Exception $e) {
            }
        }
        ...
        if($options['watermark'] && in_array($file->getMimeType(), $image_mime_types)) {
            UploadFilesHelper::appendWaterMark(public_path("storage/$options[path_to_save]/$filename"));
        }
     ...
     */
    public static function appendWaterMark($imagePath)
    {
        ini_set('memory_limit', '512M');
        // Check if image file exists
        if (!file_exists($imagePath)) {
            throw new Exception("Something went wrong while appending the watermark... Please try again!");
        }
        ...
        // Load watermark image and get dimensions
        $settings = Setting::first();
        $watermarkPath = $settings->website_watermark ? public_path("storage/uploads/website/$settings->website_watermark"):  public_path('images/default/watermark.png');
        ...
        // Calculate the size of the watermark image as a percentage of the background image size
        $watermarkSize = min($width, $height) * 0.2; // Change this value to adjust the size of the watermark
        $watermarkWidthNew = $watermarkSize / $watermarkWidth * $watermarkWidth;
        $watermarkHeightNew = $watermarkSize / $watermarkHeight * $watermarkHeight;

        // Resize the watermark image
        $watermarkResized = imagecreatetruecolor($watermarkWidthNew, $watermarkHeightNew);
        imagecopyresampled($watermarkResized, $watermark, 0, 0, 0, 0, $watermarkWidthNew, $watermarkHeightNew, $watermarkWidth, $watermarkHeight);

        // Calculate the position of the watermark image with padding
        $padding = 5;
        $watermarkX = $padding; // Change this value to adjust the horizontal position of the watermark
        $watermarkY = $height - $padding - $watermarkHeightNew; // Change this value to adjust the vertical position of the watermark

        // Add the watermark image to the background image
        imagecopymerge($img, $watermarkResized, $watermarkX, $watermarkY, 0, 0, $watermarkWidthNew, $watermarkHeightNew, 50);

        // Save modified image to disk and overwrite original file
