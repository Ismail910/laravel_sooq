...
<form id="validate-form" class="row" enctype="multipart/form-data" method="POST" action="{{route('admin.directory.store')}}">
    @csrf
    <div class="mt-5 mb-3 d-flex flex-wrap flex-row" style="direction: @if(app()->getlocale() == "ar") rtl @else ltr @endif" style="">
        <div class="mb-5">
    ...
    <div class="d-flex data flex-wrap align-items-center justify-content-between" style="direction: @if(app()->getlocale() == "ar") rtl @else ltr @endif">
        <div class="col-12 col-sm-4 mb-3">
                    ...
                    <div class="tab-pane fade show {{ $loop->first ? 'active' : '' }}" id="pills-{{ $key }}" role="tabpanel" aria-labelledby="pills-{{ $key }}-tab">
                        <div class="col-12 col-lg-12 p-2">
                            ...
                            <div class="col-12 pt-3">
                                <input type="text" name="{{ $key }}[name]" required data-name class="form-control" value="{{old($key . '.name')}}">
                            </div>
                            <div class="col-12">
                                {{__("lang.Page Desc")}}
                            </div>
                            <div class="col-12 pt-3">
                                <textarea name="{{ $key }}[description]" required data-description class="form-control" style="resize: none;"  placeholder="{{ __("lang.Page Desc Placeholder")}}" rows="5">{{old($key . '.description')}}</textarea>        
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <input type="number" class="form-control" required name="phone" id="phone" value="{{old("phone")}}">
        </div>
    ...
    <div class="col-12 col-sm-4 mb-3 d-flex flex-column justify-content-center w-100" style="text-align: center;">
