...
<form id="validate-form" class="row" enctype="multipart/form-data" method="POST" action="{{route('admin.directory.update',$directory->id)}}">
    @csrf
    @method("PUT")
    <div class="mt-5 mb-3 d-flex flex-wrap justify-content-between flex-row" style="direction: @if(app()->getlocale() == "ar") rtl @else ltr @endif">
        <div class="mb-5">
    ...
    <div class="row row-cols-3 data" style="direction:@if(app()->getlocale() == "ar") rtl @else ltr @endif">
                ...
                @foreach (config("laravellocalization.supportedLocales") as $key => $lang)
                    <div class="tab-pane fade show {{ $loop->first ? 'active' : '' }}" id="pills-{{ $key }}" role="tabpanel" aria-labelledby="pills-{{ $key }}-tab">
                        <div class="col-12 col-lg-12 p-2">
                            ...
                            <div class="col-12 pt-3">
                                <input type="text" name="{{ $key }}[address]" required data-address value="{{$directory->getTranslation('address', $key, false)}}" class="form-control">          
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="col-sm-12 col-md-6 col-lg-7">
            ...
            <input type="file" name="license"  id="license" value="{{$directory->license}}" class="form-control" accept="image/png, image/jpg, image/jpeg, image/webp">
