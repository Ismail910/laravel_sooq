                        ...
                        <div class="tab-content" id="pills2-tabContent">
                            @foreach (config('laravellocalization.supportedLocales') as $key2 => $lang)
                                <div class="tab-pane fade show {{ $loop->first ? 'active' : '' }}"
                                    ...
                                    <input id="attrr" class="form-control mb-3" type="text" name="[{{ $key2 }}.value]" placeholder="{{ $attr->getTranslation('name', $key2) }}" value="{{ $catAttributes->AnnouncementAttribute[$loop->index]->value ?? '' }}" />
                                </div>
                            @endforeach
                        </div>
                    </div>
