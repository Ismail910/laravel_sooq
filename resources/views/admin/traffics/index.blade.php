...
<div class="col-12   row p-4" style="padding: 30px 0px;position: relative;background: #fff">
    <table id="myTable" class="table table-striped table-bordered col-12 " style="padding: 0px;">
        ...
        <tbody>
            ...
            <tr>
                <td style="font-size: 12px;@if($traffic->user_id != null)background: green;@endif">
                    @if($traffic->user_id!=null)
                        @php$get_user=\App\Models\User::where('id',$traffic->user_id)->first();@endphp
                        <a href="{{route('user.index',$get_user)}}" target="_blank">{{$traffic->id}}</a>
                    @else
                        {{$traffic->id}}
                    @endif
                </td>
                ...
                <td style="font-size: 12px">
                    <a href="{{$traffic->prev_link}}">{{$traffic->domain}}</a>
                </td>
                <td style="font-size: 12px">{{$traffic->ip}}</td>
                <td style="font-size: 9px;max-width: 100px;">{{$traffic->device}}</td>
